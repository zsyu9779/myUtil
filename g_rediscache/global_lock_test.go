...
func ExampleUseGLock() {
	...
	res, err := UseGLock(context.Background(), cacheKey).WithTimeout(-1).WithExpire(10 * time.Second).Then(func() (i interface{}, e error) {
		return 1, e
	})
...
func TestUseGLock_BizResult(t *testing.T) {
	...
	expected := "ha"
	res, err := UseGLock(context.Background(), cacheKey).WithTimeout(0).WithExpire(10 * time.Second).Then(func() (i interface{}, e error) {
		return expected, nil
	})
	if err != nil {
...
func TestUseGLock_Release(t *testing.T) {
	...
	t.Log("cacheKey:", cacheKey)
	go func() {
		_, err := UseGLock(context.Background(), cacheKey).WithTimeout(-1).WithExpire(10 * time.Second).Then(func() (i interface{}, e error) {
	...
	res, err := GetRedisClient().Get(context.Background(), cacheKey).Result()
	if err == redis.Nil {
