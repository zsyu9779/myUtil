...
func ExampleUseHashAop() {
	...
	_, fromCaChe, _ := UseHashAop(context.Background(), cacheKey, reflect.TypeOf(User{}), []string{"1", "2"}, "Id").WithExpires(5 * time.Second).Then(bizFunc)
...
func TestHashAop(t *testing.T) {
	...
	// 查看回填情况
	cacheVs, err := GetRedisClient().HMGet(context.Background(), cacheKey, "1", "2").Result()
