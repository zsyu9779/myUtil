...
type UnboundedChan struct {
	In     chan<- chTask // channel for write
	Out    <-chan chTask // channel for read
	buffer []chTask      // buffer
}
...
func NewUnboundedChan(initCapacity int) UnboundedChan {
	// 创建三个字段和无限缓存的chan类型
	in := make(chan chTask, initCapacity)
	out := make(chan chTask, initCapacity)
	ch := UnboundedChan{In: in, Out: out, buffer: make([]chTask, 0, initCapacity)}
			...
			for len(ch.buffer) > 0 {
				...
				case out <- ch.buffer[0]: // 把缓存中最老的数据放入到out中，并移出第一个元素
					...
					if len(ch.buffer) == 0 { // 避免内存泄露. 如果缓存处理完了，恢复成原始的状态
						ch.buffer = make([]chTask, 0, initCapacity)
					}
				}
			}
		}
